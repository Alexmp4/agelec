...
/* ================== src/js/libs/averta.livetabs.js =================== */
/**
 * AvertaLiveTabs v1.4
...
/* ================== src/js/libs/averta.rangeslider.js =================== */
...
(function($){
	...
	var p = rangeSlider.prototype;

	p.addRange = function(id , value , classname , labelProxy){
		var element = $('<div></div>').width(value).appendTo(this.$element).addClass(classname);
		var tt = $('<div></div>').addClass('avt-range-tooltip')//.appendTo(element);
		var range = {element: element , value : value , id : id , labelProxy:labelProxy , tt:tt}, 
			that = this;
		...
		element.mousedown(function(event) {
			...
			$this.addClass('rangeslider-start');

			that._startX = event.pageX;
			that._startW = $this.width();
			that._draging = $this;
			that._start = true;

			$(document).bind('mousemove' , {ref:that} , that._onMouseMove)
					   .bind('mouseup' , {ref:that} , that._onMouseUp);

			if(that.onStart)
				that.onStart.call(that.onStartRef , range);

		}).mouseover(function(event) {
			if(that._start) return;
			tt.stop(true).fadeIn(150);
			tt.appendTo('body');
			that._updateTT(range);

			// $(this).bind('mousewheel' , function(event){
			// 	that.setValue(range ,range.value - event.deltaY );
			// 	that._updateTT(range);
			// 	event.preventDefault();
			// });
			 ...
			 if(!that._start)
				 tt.stop(true).fadeOut(150 , function(){
				 	tt.detach();
				 });
	...
	p.hideRange = function(range){
		range.element.css('display' , 'none');
	};

	p.showRange = function(range){
		range.element.css('display' , '');
	};

	p.removeRange = function(range) {
	...
	p.disable = function(){
		if( this.disabled ) return;
 		...
 		this.$element.addClass('avt-disabled');
	};
	...
	p.enable = function(){
		...
		this.$element.removeClass('avt-disabled');
	};

	p._updateTT = function(range){
		var tt = range.tt,
			ele_pos = range.element.offset();
		...
		tt.html(range.labelProxy.call(null , range));
	}

	p._onMouseMove = function(event){
		var that = event.data.ref;
		var range = that._draging.data('range');
        
        that.setValue(range , Math.max(0 , that._startW + (event.pageX - that._startX)));

        if(that.onMove)
        	that.onMove.call(that.onMoveRef , that._draging.data('range'));
	...
	p._onMouseUp = function(event){
		...
		$(document).unbind('mousemove' , that._onMouseMove)
				   .unbind('mouseup'   , that._onMouseUp);

	};

	averta.rangeSlider = rangeSlider;

})(jQuery);

/* ================== src/js/libs/averta.aligner.js =================== */
;(function(){
...
/* ================== src/js/libs/averta.jtween.js =================== */
//jtween\JTween.js 
 
/**
* JTween v0.1 by Averta. Feb, 2013
* Copyright (c) 2013 Averta
*  
* Based on GTween v2.01 by GSkinner
**/
...
;(function(window){
	...
	var JTween = function(target, duration, values, props, pluginData){
		...
		**/
		this.pluginData = this.__copy(pluginData,{});
		...
		**/
		this.dispatchEvents = JTween.defaultDispatchEvents;
		...
		**/
		...
		this.__copy(props,this);
		this.resetValues(values);
		
		if (swap) this.swapValues();
		if (this.duration === 0 && this.delay === 0 && autoPlay) this.setPosition(0);
		
	};
	
	var p = JTween.prototype;
	...
	**/
	JTween.timeScaleAll = 1;
	
	/** @private **/
	JTween.hasStarPlugins = false;
	/** @private **/
	JTween.plugins = {};
	/** @private **/
	JTween.time = 0;
	...
	**/
	JTween.installPlugin = function(plugin, propertyNames, highPriority) {
		for (var i=0; i<propertyNames.length; i++) {
			var propertyName = propertyNames[i];
			if (propertyName === "*") { JTween.hasStarPlugins = true; }
			if (JTween.plugins[propertyName] == null) { JTween.plugins[propertyName] = [plugin]; continue; }
			if (highPriority) {
				JTween.plugins[propertyName].unshift(plugin);
			} else {
				JTween.plugins[propertyName].push(plugin);
			}
		}
	};
	
	/** @private **/
	JTween.staticInit = function() {
		requestAnimationFrame(JTween.staticTick);
		JTween.time = Date.now()/1000;
		JTween.staticTick();
	};
	...
	/** @private **/
	JTween.staticTick = function(t3) {
		var t = JTween.time;
		JTween.time = Date.now()/1000;
		if (JTween.pauseAll) { return; }
		var dt = (JTween.time-t)*JTween.timeScaleAll;
		
		for (var key in JTween.tickList) {
			var tween = JTween.tickList[key];
			tween.position(tween._position+(tween.useFrames?JTween.timeScaleAll:dt)*tween.timeScale);
		}
		requestAnimationFrame(JTween.staticTick);
	};
	...
	**/
	p.paused = function(value) {
		...
		if (value) {
			delete(JTween.tickList[this.id]);
			
			// EDIT
			//if (this.target is IEventDispatcher) { this.target.removeEventListener("_", invalidate); }
			delete(JTween.gcLockList[this.id]);
		} else {
			...
			JTween.tickList[this.id] = this;
			// prevent garbage collection:
			// EDIT
			//if (target is IEventDispatcher) { target.addEventListener("_", invalidate); }
			//else { JTween.gcLockList[this] = true; }
			JTween.gcLockList[this.id] = this;
		}
	...
	**/
	p.position = function(value) {
		// get
		if(value === null) return this._position;
		...
		if (this.target && (this._position >= 0 || this.positionOld >= 0) && this.calculatedPosition != this.calculatedPositionOld) {
			...
			for (var n in this._values) {
				...
				pluginArr = JTween.plugins[n];
				if (pluginArr) {
					for (var i=0 , l = pluginArr.length; i<l; i++) {
						val = pluginArr[i].tween(this,n,val,initVal,rangeVal,this.ratio,end);
					}
					if (!isNaN(val)) { this.target[n] = val; }
				} else {
					this.target[n] = val;
				}
			}
		...
		if (JTween.hasStarPlugins) {
			pluginArr = JTween.plugins["*"];
		...
		if (!this.suppressEvents) {
			if (this.dispatchEvents) { this.__dispatchEvt("change"); }
			if (this.onChange != null) { this.onChange(this); }
		}
		if (end) {
			this.paused(true);
			if (this.nextTween) { this.nextTween.paused(false); }
			if (!this.suppressEvents) {
				if (this.dispatchEvents) { this.__dispatchEvt("complete"); }
	...
	**/
	p.delay = function(value) {
		//get 
		if(value === null) return this._delay;
	...
	**/
	p.setValues = function(values) {
		this.__copy(values,this._values,true);
		this.__invalidate();
	};
	...
	**/
	p.getValues = function() {
		return this.__copy(this._values, {});
	};
	...
	**/
	p.init = function() {
		...
		for (var n in this._values) {
			if (JTween.plugins[n]) {
				var pluginArr = JTween.plugins[n];
				var l = pluginArr.length;
		...
		if (!this.suppressEvents) {
			if (this.dispatchEvents) { this.__dispatchEvt("init"); }
	...
	**/
	...
	/** @private **/
	p.__dispatchEvt = function(name) {
		//EDIT
		//if (hasEventListener(name)) { dispatchEvent(new Event(name)); }
	};
	
	/**
	 * generate tranform object from transform string
	 * @param  {string} transformCSS_str 
	 * @return {object}                  
	 */
	JTween.parseTransform = function(transformCSS_str) {
		if(transformCSS_str === null) return {};
	...
	/**
	 * generate reset transform
	 * @static
	 * @param {string} transform
	 * @return {string} reset transform
	 */
	JTween.resetTransform = function(transform){
		var transform_obj = JTween.parseTransform(transform),
 			return_trans = '' , i , l;

 		for(var key in transform_obj){

 			return_trans += key + '('; // name(
 			
 			if(key === 'perspective'){ // don't change the perspective value
 				return_trans += transform_obj[key][0];
 			}else if(key.indexOf('scale') !== -1){
 				for(i = 0 , l = transform_obj[key].length; i !== l ; i++){
	 				return_trans += (i !== l-1 ? '1,' : '1');
	 			}
 			}else{
	 			for(i = 0 , l = transform_obj[key].length; i !== l ; i++){
	 				return_trans += (i !== l-1 ? '0,' : '0');
	 			}
 			}
...
;(function(window){
	...
	Linear.easeNone = function(ratio, unused1, unused2, unused3) {
		return ratio;
	}
...
;(function(window){
	...
	window.Sine = Sine;
})(window);//jtween\JTweenTimeline.js 
...
;(function(window){
	...
	**/
	var JTweenTimeline = function(target, duration, values, props, pluginData, tweens) {
		duration = duration || 1;
		...
		**/
		...
		JTween.call(this, target, duration, values, props, pluginData);
		// unlike GTween, which waits for a setValue to start playing, GTweenTimeline starts immediately:
		
		if (this.autoPlay) { this.paused(false); }
	};
	
	JTweenTimeline.prototype = new JTween(-1);
	var p =  JTweenTimeline.prototype;
	JTweenTimeline.prototype.constructor = JTweenTimeline;
	var _super = JTween.prototype;
	...
	**/
	JTweenTimeline.setPropertyValue = function(target,propertyName,value) {
	...
	p.position = function(value) {
		...
		// run all the normal JTween logic:
		_super.position.call(this , value);
		
		//console.log(this , this.tweens);
		
		// update tweens:
		var repeatIndex = this._position/this.duration>>0;
		var rev = (this.reflect && this.repeatIndex%2>=1);
		var i,l = this.tweens.length;
		
		if (rev) {
			for (i=0; i<l; i++) {
				this.tweens[i].position(this.calculatedPosition - this.tweenStartPositions[i]);
			}
		} else {
			for (i=l-1; i>=0; i--) {	
				this.tweens[i].position(this.calculatedPosition-this.tweenStartPositions[i]);
		...
		if (!this.suppressCallbacks) { this.checkCallbacks(); }
		
		// handle events now that everything is complete:
		this.suppressEvents = this.tmpSuppressEvents;
		if (this.onChange != null && !this.suppressEvents) { this.onChange(this); }
		if (this.onComplete != null && !this.suppressEvents && value >= this.repeatCount*this.duration && this.repeatCount > 0) { this.onComplete(this); }
	};
	...
	**/
	p.addTween = function(position,tween) {
	...
	**/
	p.addTweens = function(tweens) {
		if (tweens == null) { return; }
		for (var i=0; i<tweens.length; i+=2) {
			this.addTween(tweens[i],tweens[i+1]);
		}
	};
	...
	**/
	p.removeTween = function(tween) {
		for (var i=this.tweens.length; i>=0; i--) {
	...
	**/
	p.addLabel = function(position,label) {
	...
	**/
	p.addCallback = function(labelOrPosition, forwardCallback, forwardParameters, reverseCallback, reverseParameters) {
		var position = this.resolveLabelOrPosition(labelOrPosition);
		if (isNaN(position)) { return; }
		var callback = new Callback(position, forwardCallback, forwardParameters, reverseCallback, reverseParameters);
		var l = this.callbacks.length;
		for (var i = l-1; i>=0; i--) {
			if (position > this.callbacks[i].position) { break; }
		}
	...
	**/
	p.removeCallback = function(labelOrPosition) {
		var position = this.resolveLabelOrPosition(labelOrPosition);
		if (isNaN(position)) { return; }
		var l = this.callbacks.length;
		for (var i = 0; i<l; i++) {
	...
	**/
	p.gotoAndPlay = function(labelOrPosition) {
	...
	* @param labelOrPosition The label name or numeric position in frames or seconds (as per the timing mode of this tween) to jump to.
	**/
	p.gotoAndStop = function(labelOrPosition){
		this.goto(labelOrPosition);
		this.paused(true);
	}
	...
	* @param labelOrPosition The label name or numeric position in frames or seconds (as per the timing mode of this tween) to jump to.
	**/
	p.goto = function(labelOrPosition) {
	...
	**/
	p.resolveLabelOrPosition = function (labelOrPosition) {
	...
	**/
	...
	/** @private **/
	p.checkCallbackRange = function(startPos,endPos,includeStart) {
	...
	var Callback = function( position, forward, forwardParams, reverse, reverseParams ){
	...
	window.JTweenTimeline = JTweenTimeline;
	
})(window);
//jtween\plugins\CSSPlugin.js 
...
;(function(window, $){
	...
	CSSPlugin.enabled = true;
	CSSPlugin.inistalled = false;
	CSSPlugin.autoInstall = true;
	CSSPlugin.transformFallback = true;
	
	var cssPrefix;// = window._jcsspfx;//getVendorPrefix();

	var parseTransform = function(transformCSS_str) {
		if(transformCSS_str === null) return {};
	...
	var getTransformUnit = function(key){
		var unit = 'px';
					
		if(key.indexOf('rotate') !== -1 || key.indexOf('skew') !== -1) unit = 'deg';
		else if(key.indexOf('scale') !== -1)						   unit = '';
	...
	var tweenProperties = [
			...
			// position
			'left' , 'right' , 'top' , 'bottom',   // px , % , mm , em , ... 
			
			// dimension 
			'width' , 'height' , 'maxWidth' , 'minWidth' , 'maxHeight' , 'minHeight', // px , % , mm , em , ... 
			
			// margin 				   
			'margin', 'marginTop' , 'marginBottom' , 'marginLeft' , 'marginRight', // px , % , mm , em , ... 
			
			// padding
			'padding', 'paddingTop', 'paddingBottom' , 'paddingLeft', 'paddingRight', // px , % , mm , em , ... 
			
			// font and text style
			'fontSize' , 'fontWight' , 'lineHeight' , 'wordSpacing', 'textIndent' , 'letterSpacing', // px , % , mm , em , ... 
			
			// colors
			'color' , 'backgroundColor' , 'borderColor' , 'outlineColor', 'borderTopColor' , 'borderBottomColor' , 'boderLeftColor' , 'borderRightColor', // #FFF , #FFFFFF , rgb() , rgba()
			
			// border / outline
			'borderWidth' , 'borderTopWidth' , 'borderBottomWidth' , 'boderLeftWidth' , 'borderRightWidth', 'outlineWidth', // px , % , mm , em , ... 

			// other
			'zoom' , 'zIndex' , 'opacity' ,	'transform' , 'borderRadius'					
	];
	
	var getStyle = CSSPlugin.getStyle = function(tar , name){
		var r_value = tar.css(name);
		if(r_value === 'auto' || r_value === 'none')
			return "0px";

		return r_value;
	};
	
	var convertToPx = function(target, value , name){
		if(value.indexOf('px') !== -1) 
			return value;
		
		var current = target.css(name);
		var r_value = target.css(name , value).css(name);
		target.css(name , current);
		return r_value;
	};
	
	CSSPlugin.parseColor = function(value){
		// #FFFFFF / #FFF
		if(value.charAt(0) === '#'){
			...
			return {type:'color'  , r:value >> 16, g:(value >> 8) & 255 , b:value & 255 };
		}
		...
		if(value.charAt(0) === 'r'){
			...
			var parts_l = parts.length;
			var r_value = {type:'color'};
			
			r_value.r = parseInt(parts[0].slice(parts_l + 1));
			r_value.g = parseInt(parts[1]);
			
			if(parts_l === 4){
				r_value.b = parseInt(parts[2]);
				r_value.a = parseFloat(parts[3].slice(0 , parts[3].length - 1));
			}else{
				r_value.b = parseInt(parts[2].slice(0 , parts[2].length - 1));
			}
	...
	CSSPlugin.install = function(){
		...
		if(CSSPlugin.inistalled) return;
		
		CSSPlugin.inistalled = true;
		
		JTween.installPlugin(CSSPlugin , tweenProperties , true);
	};
	
	if(CSSPlugin.autoInstall) CSSPlugin.install();
	
	
	CSSPlugin.init = function(tween , name  , value){
		cssPrefix = window._jcsspfx;
		tween.css_data 		=  tween.css_data 		|| {};
		tween.init_css_data =  tween.init_css_data  || {};
		
		var $target = $(tween.target);
		
		if(isNaN(value))
			value = tween._values[name];
		...
		if(typeof(value) === "string" ){
			...
			// Colors
			if(str_value.charAt(0) === '#' || str_value.slice(0 , 3) === 'rgb'){
				tween.init_css_data[name] = CSSPlugin.parseColor(getStyle($target , name));
				
				var to_value = CSSPlugin.parseColor(str_value);
				
				to_value.r -= tween.init_css_data[name].r;
				to_value.g -= tween.init_css_data[name].g;
				to_value.b -= tween.init_css_data[name].b;
				if(to_value.a)	to_value.a -= tween.init_css_data[name].a;
				
				// r , g , b range values
				tween.css_data[name] = to_value;				
			...
			if(name === 'transform'){
				...
				var trans_obj 		   = parseTransform(str_value);
				var current_trans_obj  = parseTransform(tween.target.style[cssPrefix + 'Transform']);
			//	console.log(trans_obj , current_trans_obj , tween.target.style[cssPrefix + 'Transform'] , cssPrefix + 'Transform')
				tween.init_css_data['transform'] = current_trans_obj;
				
				for(var key in trans_obj){
					
					if(key in current_trans_obj){
						for(var i = 0 , l = trans_obj[key].length ; i < l ; ++i )
							trans_obj[key][i] = parseFloat(trans_obj[key][i]) - parseFloat(current_trans_obj[key][i]);
					}else{
						for(var i = 0 , l = trans_obj[key].length ; i < l ; ++i )
							trans_obj[key][i] = parseFloat(trans_obj[key][i]);
					}
					
				}
				
				tween.css_data['transform'] = trans_obj;
			...
			if(value != str_value){
				tween.css_data[name] = 'px';
				tween._values[name] = parseFloat(convertToPx($target , str_value , name));
			}else{
				tween._values[name] = value;
			}
		}
				
		return parseFloat(getStyle($target , name)) || 0;
		
	};
	
	CSSPlugin.tween = function(tween, name, value, initValue, rangeValue, ratio, end) {
		
		var $target = $(tween.target);
		
		if(tween.css_data[name]){
			if(name === 'opacity'){
				$(tween.target).css('opacity' , initValue + rangeValue*ratio);
				
			}else if(typeof(tween.css_data[name]) === 'object'){
				
				if(tween.css_data[name].type === 'color'){
					if(tween.css_data[name].a){
						tween.target.style[name] = 'rgba(' + Math.floor(tween.init_css_data[name].r + tween.css_data[name].r * ratio) + ',' +
															 Math.floor(tween.init_css_data[name].g + tween.css_data[name].g * ratio) + ',' +
															 Math.floor(tween.init_css_data[name].b + tween.css_data[name].b * ratio) + ',' +
															 tween.init_css_data[name].a + tween.css_data[name].a * ratio + ')';
					}else{
						tween.target.style[name] = 'rgb(' + Math.floor(tween.init_css_data[name].r + tween.css_data[name].r * ratio) + ',' +
															Math.floor(tween.init_css_data[name].g + tween.css_data[name].g * ratio) + ',' +
															Math.floor(tween.init_css_data[name].b + tween.css_data[name].b * ratio) + ')';
					}
				...
				if(name === 'transform'){
					...
					for(var key in tween.css_data['transform']){
						unit = getTransformUnit(key);
						
						trans_str += key + '(';
						
						for(var i = 0 , l = tween.css_data['transform'][key].length ; i !== l ; ++i ){
							
							trans_str += ((tween.init_css_data['transform'][key] ? parseFloat(tween.init_css_data['transform'][key][i]) : 0) + tween.css_data['transform'][key][i] * ratio) + unit + (i !== l - 1 ? ' ,' : '');
						}
				...
				//	console.log(trans_str)
					...
					tween.target.style[cssPrefix + 'Transform'] = trans_str;
					
					return NaN;
				}
				
			}else{
				tween.target.style[name] = initValue + rangeValue*ratio + tween.css_data[name];
			}
			
			return NaN;
		}
		
		//console.log(name , initValue + rangeValue*ratio);
		
		tween.target.style[name] = String(initValue + rangeValue*ratio);
		return NaN;
	};
...
(function($){
	 ...
	 */
 	GFonts.getList = function(onLoad){
 		...
 		$.getJSON( url + '?key=' + api, function(data){
 			list = data;
 	...
 	GFonts.load = function(font, variants){
 		if( GFonts.loadedFonts.indexOf(font) === -1 ){
 			$('head').append('<link rel="stylesheet" href="' + (location.protocol || 'http:') + '//fonts.googleapis.com/css?family=' + escape(font) + ':' + variants + '" >');
...
/* ================== src/js/libs/averta.em-offline-adapter.js =================== */
/**
